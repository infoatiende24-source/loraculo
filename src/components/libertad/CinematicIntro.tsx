...
export default function CinematicIntro() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", watchReturnToStart, { passive: true });
    watchReturnToStart();
    return () => window.removeEventListener("scroll", watchReturnToStart);
  }, [active]);
  ...
  return (
    <div
      ...
      style={{ opacity: closing ? 0 : 1, backgroundColor: `rgba(2, 2, 2, ${1 - reveal})`, transition: "opacity 1050ms cubic-bezier(0.22, 1, 0.36, 1), background-color 380ms cubic-bezier(0.22, 1, 0.36, 1)" }}
    >
      ...
      <div className="absolute inset-0" style={{ opacity: signal, transition: smooth }}>
        {sparks.map(([left, top], index) => <i key={index} className="absolute h-1 w-1 rounded-full bg-[#f6dc8a] shadow-[0_0_10px_rgba(244,201,90,0.95)]" style={{ left: `${left}%`, top: `${top}%`, opacity: 0.35 + (index % 3) * 0.18, transform: `scale(${0.5 + signal * 0.8})`, transition: smooth }} />)}
      </div>

      <div className="absolute left-1/2 top-1/2 h-[110vmin] w-[110vmin] -translate-x-1/2 -translate-y-1/2">
        <div className="absolute inset-[6%] rounded-full border border-[#f4d982]/80 shadow-[0_0_32px_8px_rgba(228,177,55,0.18)]" style={{ opacity: portal, transform: `scale(${0.06 + portal * 0.94}) rotate(${portal * 45}deg)`, transition: smooth }} />
        <div className="absolute inset-[18%] rounded-full border border-[#d9a649]/75" style={{ opacity: portal * 0.9, transform: `scale(${0.04 + portal * 0.96}) rotate(${-portal * 62}deg)`, transition: smooth }} />
        <div className="absolute inset-[30%] rounded-full border border-[#fff0b5]/55" style={{ opacity: portal * 0.72, transform: `scale(${0.02 + portal * 0.98}) rotate(${portal * 80}deg)`, transition: smooth }} />
        <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,rgba(255,244,184,0.95)_0%,rgba(231,184,65,0.38)_15%,rgba(191,99,117,0.12)_33%,transparent_66%)]" style={{ opacity: portal * 0.86, transform: `scale(${0.08 + portal * 0.96})`, transition: smooth }} />
      </div>

      <div className="absolute left-1/2 top-1/2 z-10 w-full -translate-x-1/2 -translate-y-1/2 px-8 text-center" style={{ opacity: phraseOpacity, transition: smooth }}>
      ...
      <div className="absolute bottom-12 left-1/2 z-20 w-full -translate-x-1/2 px-6 text-center" style={{ opacity: reveal * clamp((0.97 - progress) * 14), transition: smooth }}>
      ...
      <div className="absolute bottom-8 left-1/2 z-20 -translate-x-1/2 text-center" style={{ opacity: promptOpacity, transition: smooth }}>
